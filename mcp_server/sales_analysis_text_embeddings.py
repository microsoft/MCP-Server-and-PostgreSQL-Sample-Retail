...
class SemanticSearchTextEmbedding:
    ...
    def __init__(self) -> None:
        ...
        logger.info("AZURE_OPENAI_ENDPOINT = '%s'", self.endpoint)
    ...
    def _setup_azure_openai_client(self) -> AzureOpenAI:
        """Setup and return Azure OpenAI client with Azure AD authentication."""
        if not self.endpoint:
            raise ValueError("Azure OpenAI endpoint is not configured")

        api_version = "2024-02-01"
        
        # Use DefaultAzureCredential which will use service principal env vars if available
