...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T18:07:15+00:00",
  "source_file": "README.md",
  "language_code": "hi"
}
-->
# MCP सर्वर और PostgreSQL सैंपल - रिटेल सेल्स एनालिसिस

## डेटाबेस इंटीग्रेशन के साथ MCP सीखें - प्रैक्टिकल उदाहरणों के माध्यम से
...
[![Azure AI Foundry Discord से जुड़ें](https://dcbadge.limes.pink/api/server/ByRwuEEgH4)](https://discord.com/invite/ByRwuEEgH4)
...
यह सैंपल दिखाता है कि कैसे एक व्यापक **Model Context Protocol (MCP) सर्वर** बनाया और तैनात किया जा सकता है, जो AI असिस्टेंट्स को PostgreSQL के माध्यम से रिटेल सेल्स डेटा तक सुरक्षित और बुद्धिमान पहुंच प्रदान करता है। यह प्रोजेक्ट एंटरप्राइज-ग्रेड फीचर्स जैसे **Row Level Security (RLS)**, **सेमांटिक सर्च क्षमताएं**, और **Azure AI इंटीग्रेशन** को वास्तविक दुनिया के रिटेल एनालिटिक्स परिदृश्यों के लिए प्रदर्शित करता है।

**मुख्य उपयोग के मामले:**
- **AI-संचालित सेल्स एनालिटिक्स**: AI असिस्टेंट्स को प्राकृतिक भाषा के माध्यम से रिटेल सेल्स डेटा को क्वेरी और एनालाइज करने में सक्षम बनाना
- **सुरक्षित मल्टी-टेनेंट एक्सेस**: Row Level Security का प्रदर्शन, जहां विभिन्न स्टोर मैनेजर्स केवल अपने स्टोर का डेटा देख सकते हैं
- **सेमांटिक प्रोडक्ट सर्च**: टेक्स्ट एम्बेडिंग का उपयोग करके AI-संवर्धित प्रोडक्ट डिस्कवरी
- **एंटरप्राइज इंटीग्रेशन**: MCP सर्वर को Azure सेवाओं और PostgreSQL डेटाबेस के साथ एकीकृत करने का प्रदर्शन
...
- AI एप्लिकेशन डेवलपर्स जो रिटेल या ई-कॉमर्स डेटा के साथ काम कर रहे हैं
- कोई भी जो AI असिस्टेंट्स को एंटरप्राइज डेटाबेस के साथ जोड़ने में रुचि रखता है

## Azure AI Foundry Discord समुदाय से जुड़ें
MCP के अनुभव साझा करें और विशेषज्ञों और प्रोडक्ट ग्रुप्स से मिलें 

[![Azure AI Discord](https://dcbadge.limes.pink/api/server/kzRShWzttr)](https://discord.gg/kzRShWzttr)

# सेल्स एनालिसिस MCP सर्वर

एक Model Context Protocol (MCP) सर्वर जो Zava Retail DIY Business के लिए व्यापक ग्राहक सेल्स डेटाबेस एक्सेस प्रदान करता है। यह सर्वर AI असिस्टेंट्स को एक सुरक्षित, स्कीमा-अवेयर इंटरफेस के माध्यम से रिटेल सेल्स डेटा को क्वेरी और एनालाइज करने में सक्षम बनाता है।

## 📚 संपूर्ण कार्यान्वयन गाइड

इस समाधान को कैसे बनाया गया है और इसी तरह के MCP सर्वर को कैसे लागू किया जाए, इसके विस्तृत विवरण के लिए हमारी **[सैंपल वॉकथ्रू](Sample_Walkthrough.md)** देखें। यह गाइड प्रदान करता है:

- **आर्किटेक्चर डीप डाइव**: घटक विश्लेषण और डिज़ाइन पैटर्न
- **स्टेप-बाय-स्टेप बिल्डिंग**: प्रोजेक्ट सेटअप से लेकर तैनाती तक
- **कोड ब्रेकडाउन**: MCP सर्वर कार्यान्वयन का विस्तृत विवरण
- **उन्नत फीचर्स**: Row Level Security, सेमांटिक सर्च, और मॉनिटरिंग
- **सर्वोत्तम प्रथाएं**: सुरक्षा, प्रदर्शन, और विकास दिशानिर्देश
- **समस्या निवारण**: सामान्य समस्याएं और समाधान

उन डेवलपर्स के लिए आदर्श जो कार्यान्वयन विवरण को समझना चाहते हैं और इसी तरह के समाधान बनाना चाहते हैं।

## 🤖 MCP (Model Context Protocol) क्या है?

**Model Context Protocol (MCP)** एक ओपन स्टैंडर्ड है जो AI असिस्टेंट्स को वास्तविक समय में बाहरी डेटा स्रोतों और टूल्स तक सुरक्षित पहुंच प्रदान करता है। इसे एक पुल के रूप में सोचें जो AI मॉडल को डेटाबेस, APIs, फाइल सिस्टम, और अन्य संसाधनों से जोड़ता है, जबकि सुरक्षा और नियंत्रण बनाए रखता है।

### मुख्य लाभ:
- **रियल-टाइम डेटा एक्सेस**: AI असिस्टेंट्स लाइव डेटाबेस और APIs को क्वेरी कर सकते हैं
- **सुरक्षित इंटीग्रेशन**: प्रमाणीकरण और अनुमतियों के साथ नियंत्रित पहुंच  
- **टूल एक्स्टेंसिबिलिटी**: AI असिस्टेंट्स में कस्टम क्षमताएं जोड़ें
- **स्टैंडर्डाइज्ड प्रोटोकॉल**: विभिन्न AI प्लेटफॉर्म्स और टूल्स के साथ काम करता है

### MCP में नए हैं?

यदि आप Model Context Protocol में नए हैं, तो हम Microsoft के व्यापक शुरुआती संसाधनों के साथ शुरुआत करने की सलाह देते हैं:

**📖 [MCP शुरुआती गाइड](https://aka.ms/mcp-for-beginners)**

यह संसाधन प्रदान करता है:
- MCP अवधारणाओं और आर्किटेक्चर का परिचय
- अपना पहला MCP सर्वर बनाने के लिए स्टेप-बाय-स्टेप ट्यूटोरियल
- MCP विकास के लिए सर्वोत्तम प्रथाएं
- लोकप्रिय AI प्लेटफॉर्म्स के साथ इंटीग्रेशन उदाहरण
- समुदाय संसाधन और समर्थन

एक बार जब आप मूल बातें समझ लें, तो यहां वापस आएं और इस उन्नत रिटेल एनालिटिक्स कार्यान्वयन का अन्वेषण करें!

## 📚 व्यापक शिक्षण गाइड: /walkthrough

यह रिपॉजिटरी एक पूर्ण **12-मॉड्यूल शिक्षण वॉकथ्रू** शामिल करती है, जो इस MCP रिटेल सर्वर सैंपल को छोटे, स्टेप-बाय-स्टेप पाठों में विभाजित करती है। वॉकथ्रू इस कार्यशील उदाहरण को एक व्यापक शैक्षिक संसाधन में बदल देता है, जो उन डेवलपर्स के लिए आदर्श है जो डेटाबेस इंटीग्रेशन के साथ प्रोडक्शन-रेडी MCP सर्वर बनाना सीखना चाहते हैं।

### आप क्या सीखेंगे

वॉकथ्रू MCP की मूल अवधारणाओं से लेकर उन्नत प्रोडक्शन तैनाती तक सब कुछ कवर करता है, जिसमें शामिल हैं:

- **MCP मूल बातें**: Model Context Protocol और इसके वास्तविक दुनिया के अनुप्रयोगों को समझना
- **डेटाबेस इंटीग्रेशन**: Row Level Security के साथ सुरक्षित PostgreSQL कनेक्टिविटी लागू करना
- **AI-संवर्धित फीचर्स**: Azure OpenAI एम्बेडिंग के साथ सेमांटिक सर्च क्षमताएं जोड़ना
- **सुरक्षा कार्यान्वयन**: एंटरप्राइज-ग्रेड प्रमाणीकरण, प्राधिकरण, और डेटा आइसोलेशन
- **टूल डेवलपमेंट**: डेटा एनालिसिस और बिजनेस इंटेलिजेंस के लिए उन्नत MCP टूल्स बनाना
- **टेस्टिंग और डिबगिंग**: व्यापक परीक्षण रणनीतियां और डिबगिंग तकनीकें
- **VS Code इंटीग्रेशन**: प्राकृतिक भाषा डेटाबेस क्वेरी के लिए AI चैट कॉन्फ़िगर करना
...
वॉकथ्रू एक प्रगतिशील शिक्षण संरचना का अनुसरण करता है, जो सभी कौशल स्तरों के डेवलपर्स के लिए डिज़ाइन किया गया है:

| मॉड्यूल | फोकस क्षेत्र | विवरण | समय अनुमान |
|--------|------------|-------------|---------------|
| **[00-Introduction](walkthrough/00-Introduction/README.md)** | आधार | MCP अवधारणाएं, Zava Retail केस स्टडी, आर्किटेक्चर अवलोकन | 30 मिनट |
| **[01-Architecture](walkthrough/01-Architecture/README.md)** | डिज़ाइन पैटर्न | तकनीकी आर्किटेक्चर, लेयर्ड डिज़ाइन, सिस्टम घटक | 45 मिनट |
| **[02-Security](walkthrough/02-Security/README.md)** | एंटरप्राइज सुरक्षा | Azure प्रमाणीकरण, Row Level Security, मल्टी-टेनेंट आइसोलेशन | 60 मिनट |
| **[03-Setup](walkthrough/03-Setup/README.md)** | वातावरण | Docker सेटअप, Azure CLI, प्रोजेक्ट कॉन्फ़िगरेशन, सत्यापन | 45 मिनट |
| **[04-Database](walkthrough/04-Database/README.md)** | डेटा लेयर | PostgreSQL स्कीमा, pgvector, RLS नीतियां, सैंपल डेटा | 60 मिनट |
...
| **[06-Tools](walkthrough/06-Tools/README.md)** | टूल डेवलपमेंट | MCP टूल निर्माण, क्वेरी सत्यापन, बिजनेस इंटेलिजेंस फीचर्स | 75 मिनट |
| **[07-Semantic-Search](walkthrough/07-Semantic-Search/README.md)** | AI इंटीग्रेशन | Azure OpenAI एम्बेडिंग, वेक्टर सर्च, हाइब्रिड सर्च रणनीतियां | 60 मिनट |
| **[08-Testing](walkthrough/08-Testing/README.md)** | गुणवत्ता आश्वासन | यूनिट टेस्टिंग, इंटीग्रेशन टेस्टिंग, प्रदर्शन टेस्टिंग, डिबगिंग | 75 मिनट |
| **[09-VS-Code](walkthrough/09-VS-Code/README.md)** | विकास अनुभव | VS Code कॉन्फ़िगरेशन, AI चैट इंटीग्रेशन, डिबगिंग वर्कफ़्लो | 45 मिनट |
| **[10-Deployment](walkthrough/10-Deployment/README.md)** | प्रोडक्शन रेडी | कंटेनरीकरण, Azure Container Apps, CI/CD पाइपलाइन्स, स्केलिंग | 90 मिनट |
| **[11-Monitoring](walkthrough/11-Monitoring/README.md)** | ऑब्जर्वेबिलिटी | एप्लिकेशन इनसाइट्स, संरचित लॉगिंग, प्रदर्शन मेट्रिक्स | 60 मिनट |
| **[12-Best-Practices](walkthrough/12-Best-Practices/README.md)** | प्रोडक्शन उत्कृष्टता | सुरक्षा सख्ती, प्रदर्शन अनुकूलन, एंटरप्राइज पैटर्न | 45 मिनट |

**कुल शिक्षण समय**: ~12-15 घंटे का व्यापक प्रैक्टिकल शिक्षण

### 🎯 वॉकथ्रू का उपयोग कैसे करें

**शुरुआती लोगों के लिए**:
1. MCP मूल बातें समझने के लिए [मॉड्यूल 00: परिचय](walkthrough/00-Introduction/README.md) से शुरू करें
2. पूरी शिक्षण अनुभव के लिए मॉड्यूल्स को क्रमिक रूप से फॉलो करें
...
2. उन विशेष मॉड्यूल्स पर जाएं जो आपकी रुचि रखते हैं (जैसे, मॉड्यूल 07 AI इंटीग्रेशन के लिए)
3. अपने प्रोजेक्ट्स के लिए व्यक्तिगत मॉड्यूल्स को संदर्भ सामग्री के रूप में उपयोग करें
...
3. कोड उदाहरणों को प्रोडक्शन-रेडी टेम्पलेट्स के रूप में उपयोग करें

### 🚀 त्वरित प्रारंभ विकल्प

**विकल्प 1: पूर्ण शिक्षण पथ** (शुरुआती लोगों के लिए अनुशंसित)
```bash
...
**विकल्प 2: प्रैक्टिकल कार्यान्वयन** (सीधे निर्माण में कूदें)
```bash
...
1. **हैंड्स-ऑन दृष्टिकोण**: प्रत्येक मॉड्यूल में काम करने वाले कोड उदाहरण शामिल हैं जिन्हें आप चला सकते हैं और संशोधित कर सकते हैं।
2. **क्रमिक जटिलता**: अवधारणाएँ सरल से जटिल तक धीरे-धीरे बनती हैं।
...
**अपने प्लेटफ़ॉर्म के लिए स्क्रिप्ट चुनें:**
...
2. स्टोर द्वारा बिक्री दिखाएँ।
3. पिछले तिमाही की श्रेणीवार बिक्री क्या थी?
4. "पेंट के लिए कंटेनर" के समान हम कौन से उत्पाद बेचते हैं?
...
उपयोगकर्ता क्वेरी के आधार पर उत्पादों के लिए एक सेमांटिक खोज करें।
...
- **RLS इंटीग्रेशन**: अनुरोध संदर्भ से स्वचालित उपयोगकर्ता ID संकल्प।
...
- **टेबल मान्यता**: सुनिश्चित करता है कि केवल मान्य टेबल नामों तक पहुँच हो।
- **क्वेरी मान्यता**: निष्पादन से पहले PostgreSQL क्वेरी को मान्य करता है।
...
3. **इनपुट मान्यता**: टेबल नाम और क्वेरी निष्पादन से पहले मान्य किए जाते हैं।
...
- **टूल पंजीकरण**: Pydantic मान्यता के साथ घोषणात्मक टूल पंजीकरण।
