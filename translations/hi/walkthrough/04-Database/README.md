...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T18:32:07+00:00",
...
यह मॉड्यूल Zava Retail सिस्टम के लिए PostgreSQL डेटाबेस डिज़ाइन पर गहराई से चर्चा करता है। आप एक व्यापक रिटेल स्कीमा को लागू करना सीखेंगे जिसमें वेक्टर सर्च की क्षमता, मल्टी-टेनेंट डेटा मॉडलिंग, और डेटा आइसोलेशन के लिए Row Level Security (RLS) शामिल है।

## अवलोकन

हमारा डेटाबेस MCP सर्वर की नींव है, जो कई स्टोर्स के रिटेल डेटा को संग्रहीत करता है और डेटा आइसोलेशन को सख्ती से बनाए रखता है। हम PostgreSQL का उपयोग करते हैं, जिसमें pgvector एक्सटेंशन शामिल है, जो ग्राहकों को प्राकृतिक भाषा क्वेरी का उपयोग करके उत्पाद खोजने की सुविधा देता है।

हमारा स्कीमा आधुनिक मल्टी-टेनेंट पैटर्न का अनुसरण करता है, जिसमें Row Level Security यह सुनिश्चित करता है कि उपयोगकर्ता केवल अपने अधिकृत स्टोर्स का डेटा ही एक्सेस कर सकें। यह दृष्टिकोण एंटरप्राइज़-ग्रेड सुरक्षा प्रदान करता है और प्रदर्शन को भी अनुकूल बनाए रखता है।

## सीखने के उद्देश्य

इस मॉड्यूल के अंत तक, आप सक्षम होंगे:

- **डिज़ाइन करना**: स्केलेबल मल्टी-टेनेंट रिटेल डेटाबेस स्कीमा  
- **लागू करना**: PostgreSQL को pgvector के साथ वेक्टर सर्च के लिए  
- **कॉन्फ़िगर करना**: डेटा आइसोलेशन के लिए Row Level Security  
- **जनरेट करना**: परीक्षण के लिए यथार्थवादी सैंपल डेटा  
- **अनुकूलित करना**: रिटेल वर्कलोड के लिए डेटाबेस प्रदर्शन  
- **लागू करना**: बैकअप और रिकवरी रणनीतियाँ  

## 🗃️ डेटाबेस आर्किटेक्चर

### PostgreSQL और pgvector

हमारा डेटाबेस PostgreSQL की एंटरप्राइज़ विशेषताओं का उपयोग करता है, जिसमें AI-पावर्ड सर्च के लिए pgvector एक्सटेंशन शामिल है:
...
✅ **मल्टी-टेनेंट डेटाबेस डिज़ाइन**: सुरक्षित डेटा आइसोलेशन के लिए Row Level Security लागू किया  
✅ **वेक्टर सर्च की क्षमता**: pgvector को सेमांटिक प्रोडक्ट सर्च के लिए कॉन्फ़िगर किया  
✅ **व्यापक स्कीमा**: प्रोडक्शन-रेडी रिटेल डेटाबेस स्कीमा बनाया  
✅ **सैंपल डेटा जनरेशन**: विकास और परीक्षण के लिए यथार्थवादी टेस्ट डेटा तैयार किया  
✅ **प्रदर्शन अनुकूलन**: इंडेक्स और क्वेरी ऑप्टिमाइजेशन कॉन्फ़िगर किया  
✅ **बैकअप और रिकवरी**: मजबूत डेटा सुरक्षा रणनीतियाँ स्थापित की  

## 🚀 आगे क्या करें

**[मॉड्यूल 05: MCP सर्वर कार्यान्वयन](../05-MCP-Server/README.md)** के साथ जारी रखें:

- इस डेटाबेस से कनेक्ट होने वाला FastMCP सर्वर बनाएं  
- MCP प्रोटोकॉल के लिए डेटाबेस क्वेरी टूल्स लागू करें  
- एम्बेडिंग्स का उपयोग करके सेमांटिक सर्च क्षमताएँ जोड़ें  
- कनेक्शन पूलिंग और एरर हैंडलिंग कॉन्फ़िगर करें  

## 📚 अतिरिक्त संसाधन

### PostgreSQL और pgvector
- [PostgreSQL दस्तावेज़](https://www.postgresql.org/docs/) - पूर्ण PostgreSQL संदर्भ  
- [pgvector एक्सटेंशन](https://github.com/pgvector/pgvector) - PostgreSQL के लिए वेक्टर समानता खोज  
- [PostgreSQL प्रदर्शन ट्यूनिंग](https://wiki.postgresql.org/wiki/Performance_Optimization) - अनुकूलन सर्वोत्तम प्रथाएँ  

### मल्टी-टेनेंट आर्किटेक्चर
- [Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - PostgreSQL RLS दस्तावेज़  
- [मल्टी-टेनेंट डेटा आर्किटेक्चर](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Azure आर्किटेक्चर पैटर्न  
- [डेटाबेस सुरक्षा सर्वोत्तम प्रथाएँ](https://www.postgresql.org/docs/current/security.html) - PostgreSQL सुरक्षा गाइड  

### वेक्टर डेटाबेस
- [वेक्टर सर्च की मूल बातें](https://www.pinecone.io/learn/vector-database/) - वेक्टर डेटाबेस को समझना  
- [एम्बेडिंग मॉडल्स](https://platform.openai.com/docs/guides/embeddings) - OpenAI एम्बेडिंग्स दस्तावेज़  
- [HNSW एल्गोरिथ्म](https://arxiv.org/abs/1603.09320) - हायरार्किकल नेविगेबल स्मॉल वर्ल्ड ग्राफ्स  

---

**पिछला**: [मॉड्यूल 03: एनवायरनमेंट सेटअप](../03-Setup/README.md)  
**अगला**: [मॉड्यूल 05: MCP सर्वर कार्यान्वयन](../05-MCP-Server/README.md)  
