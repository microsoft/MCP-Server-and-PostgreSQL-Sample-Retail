...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T18:47:13+00:00",
...
توجهك هذه الوحدة العملية إلى تنفيذ خادم MCP جاهز للإنتاج باستخدام إطار عمل FastMCP. ستقوم ببناء الهيكل الأساسي للخادم، تنفيذ تكامل قاعدة البيانات، إنشاء أدوات للوصول إلى البيانات، ووضع الأساس لتحليلات التجزئة المدعومة بالذكاء الاصطناعي.

## نظرة عامة

خادم MCP هو قلب حلنا لتحليلات التجزئة. يعمل كجسر بين مساعدي الذكاء الاصطناعي وقاعدة بيانات PostgreSQL، مما يوفر وصولًا آمنًا وذكيًا إلى بيانات الأعمال عبر بروتوكول موحد.

تُعلمك هذه الوحدة كيفية بناء خادم MCP قوي وقابل للتوسع وفقًا لأنماط المؤسسات وأفضل الممارسات.
...
- **سلامة النوع**: التحقق من البيانات باستخدام dataclass وتلميحات النوع  
- **الافتراضيات المرنة**: افتراضيات معقولة للتطوير  
- **التحقق**: التحقق من التكوين مع رسائل خطأ مفيدة  
- **الأمان**: القيم الحساسة فقط من متغيرات البيئة  

## 🗄️ طبقة تكامل قاعدة البيانات

### مزود PostgreSQL (`sales_analysis_postgres.py`)
...
- **استكشاف المخطط**: اكتشاف ديناميكي لمخطط الجداول  
- **معالجة الأخطاء**: إدارة شاملة للأخطاء وتسجيلها  
- **تنسيق الاستعلامات**: تنسيق النتائج بطريقة مناسبة للذكاء الاصطناعي  
...
لنقم الآن بتنفيذ خادم MCP الرئيسي:
...
- **دعم التطوير**: إعادة تحميل ساخنة وقدرات تصحيح الأخطاء  
...
قم بإنشاء اختبارات وحدات شاملة:
...
بعد إكمال هذه الوحدة، يجب أن تكون لديك:

✅ **خادم MCP يعمل**: خادم FastMCP مع تكامل قاعدة البيانات  
...
✅ **استراتيجية اختبار**: اختبار محلي وتكامل مع VS Code  

## 🚀 ما التالي

تابع مع **[الوحدة 06: تطوير الأدوات](../06-Tools/README.md)** لـ:
...
- [أفضل ممارسات تجمع الاتصالات](https://www.postgresql.org/docs/current/runtime-config-connection.html) - تحسين PostgreSQL  
...
**التالي**: جاهز لتوسيع أدواتك؟ تابع مع [الوحدة 06: تطوير الأدوات](../06-Tools/README.md)
