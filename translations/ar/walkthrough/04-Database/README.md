...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T19:02:37+00:00",
...
تتناول هذه الوحدة تصميم قاعدة بيانات PostgreSQL لنظام Zava Retail بعمق. ستتعلم كيفية تنفيذ مخطط شامل للتجزئة مع قدرات البحث باستخدام المتجهات، ونمذجة البيانات متعددة المستأجرين، وأمان مستوى الصف (RLS) لعزل البيانات.

## نظرة عامة

قاعدة البيانات هي الأساس لخادم MCP الخاص بنا، حيث تخزن بيانات التجزئة عبر العديد من المتاجر مع الحفاظ على عزل صارم للبيانات. نستخدم PostgreSQL مع امتداد pgvector لتمكين قدرات البحث الدلالي، مما يسمح للعملاء بالعثور على المنتجات باستخدام استفسارات اللغة الطبيعية.

يتبع مخططنا أنماطًا حديثة متعددة المستأجرين مع أمان مستوى الصف لضمان وصول المستخدمين فقط إلى البيانات من المتاجر المصرح لهم بها. يوفر هذا النهج أمانًا على مستوى المؤسسات مع الحفاظ على الأداء الأمثل.

## أهداف التعلم

بنهاية هذه الوحدة، ستكون قادرًا على:

- **تصميم** مخططات قاعدة بيانات تجزئة متعددة المستأجرين قابلة للتوسع  
- **تنفيذ** PostgreSQL مع pgvector للبحث باستخدام المتجهات  
- **تكوين** أمان مستوى الصف لعزل البيانات  
- **إنشاء** بيانات اختبار واقعية للتجربة  
- **تحسين** أداء قاعدة البيانات لأعباء العمل في التجزئة  
- **تنفيذ** استراتيجيات النسخ الاحتياطي والاسترداد  

## 🗃️ هندسة قاعدة البيانات
...
بعد إكمال هذه الوحدة، يجب أن تكون قد:
...
✅ **مخطط شامل**: إنشاء مخطط قاعدة بيانات تجزئة جاهز للإنتاج  
✅ **إنشاء بيانات اختبار**: بناء بيانات اختبار واقعية للتطوير والتجربة  
✅ **تحسين الأداء**: تكوين الفهارس وتحسين الاستعلامات  
✅ **النسخ الاحتياطي والاسترداد**: وضع استراتيجيات حماية البيانات القوية  

## 🚀 ما التالي

تابع مع **[الوحدة 05: تنفيذ خادم MCP](../05-MCP-Server/README.md)** لـ:
...
- [تحسين أداء PostgreSQL](https://wiki.postgresql.org/wiki/Performance_Optimization) - أفضل الممارسات لتحسين الأداء  

### هندسة متعددة المستأجرين
- [أمان مستوى الصف](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - وثائق RLS لـ PostgreSQL  
- [هندسة بيانات متعددة المستأجرين](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - أنماط هندسة Azure  
- [أفضل الممارسات لأمان قاعدة البيانات](https://www.postgresql.org/docs/current/security.html) - دليل أمان PostgreSQL  
...
**السابق**: [الوحدة 03: إعداد البيئة](../03-Setup/README.md)  
**التالي**: [الوحدة 05: تنفيذ خادم MCP](../05-MCP-Server/README.md)  
