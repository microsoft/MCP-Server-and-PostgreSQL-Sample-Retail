...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T17:13:23+00:00",
...
このモジュールでは、Zava RetailシステムのPostgreSQLデータベース設計について詳しく学びます。ベクター検索機能、多テナントデータモデリング、データ分離のためのRow Level Security (RLS) を備えた包括的なリテールスキーマを実装する方法を習得します。

## 概要

データベースはMCPサーバーの基盤であり、複数店舗のリテールデータを保存しながら厳格なデータ分離を維持します。PostgreSQLとpgvector拡張機能を使用してセマンティック検索機能を実現し、顧客が自然言語クエリを使用して商品を検索できるようにします。

スキーマは最新の多テナントパターンに従い、Row Level Securityを活用してユーザーが認可された店舗のデータのみアクセスできるようにします。このアプローチは、エンタープライズレベルのセキュリティを提供しつつ、最適なパフォーマンスを維持します。

## 学習目標

このモジュールを終了する頃には、以下ができるようになります：

- **設計**: スケーラブルな多テナントリテールデータベーススキーマを設計する  
- **実装**: PostgreSQLとpgvectorを使用してベクター検索を実現する  
- **設定**: データ分離のためのRow Level Securityを構成する  
- **生成**: テスト用のリアルなサンプルデータを作成する  
- **最適化**: リテールワークロード向けにデータベースパフォーマンスを向上させる  
- **実装**: バックアップと復旧戦略を構築する  
...
当社のデータベースは、PostgreSQLのエンタープライズ機能とpgvector拡張機能を組み合わせて、AI駆動の検索を実現しています：
...
データベースは**共有データベース、共有スキーマ**の多テナントモデルをRow Level Securityとともに使用しています：
...
このモジュールを終了した後、以下を達成しているはずです：

✅ **多テナントデータベース設計**: 安全なデータ分離のためのRow Level Securityを実装  
✅ **ベクター検索機能**: pgvectorを設定してセマンティック商品検索を実現  
✅ **包括的なスキーマ**: 本番環境対応のリテールデータベーススキーマを作成  
✅ **サンプルデータ生成**: 開発とテスト用のリアルなテストデータを構築  
✅ **パフォーマンス最適化**: インデックスとクエリ最適化を構成  
✅ **バックアップと復旧**: 信頼性の高いデータ保護戦略を確立  

## 🚀 次のステップ

**[Module 05: MCP Server Implementation](../05-MCP-Server/README.md)** に進んで以下を学びます：

- このデータベースに接続するFastMCPサーバーを構築する  
- MCPプロトコル用のデータベースクエリツールを実装する  
- 埋め込みを使用したセマンティック検索機能を追加する  
- 接続プールとエラーハンドリングを設定する  

## 📚 追加リソース

### PostgreSQL & pgvector
- [PostgreSQL Documentation](https://www.postgresql.org/docs/) - PostgreSQLの完全なリファレンス  
- [pgvector Extension](https://github.com/pgvector/pgvector) - PostgreSQL向けベクター類似性検索  
- [PostgreSQL Performance Tuning](https://wiki.postgresql.org/wiki/Performance_Optimization) - 最適化のベストプラクティス  

### 多テナントアーキテクチャ
- [Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - PostgreSQL RLSのドキュメント  
- [Multi-Tenant Data Architecture](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Azureのアーキテクチャパターン  
- [Database Security Best Practices](https://www.postgresql.org/docs/current/security.html) - PostgreSQLセキュリティガイド  

### ベクターデータベース
- [Vector Search Fundamentals](https://www.pinecone.io/learn/vector-database/) - ベクターデータベースの基礎  
- [Embedding Models](https://platform.openai.com/docs/guides/embeddings) - OpenAI埋め込みのドキュメント  
- [HNSW Algorithm](https://arxiv.org/abs/1603.09320) - 階層型ナビゲーションスモールワールドグラフ  

---

**前回**: [Module 03: Environment Setup](../03-Setup/README.md)  
**次回**: [Module 05: MCP Server Implementation](../05-MCP-Server/README.md)  

---

**免責事項**:  
本書類は、AI翻訳サービス[Co-op Translator](https://github.com/Azure/co-op-translator)を使用して翻訳されています。正確性を期すよう努めておりますが、自動翻訳には誤りや不正確な表現が含まれる可能性があります。原文（元の言語で記載された文書）が信頼できる情報源とみなされるべきです。重要な情報については、専門の人間による翻訳を推奨します。本翻訳の使用に起因する誤解や誤った解釈について、当方は一切の責任を負いません。