...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T19:27:30+00:00",
...
1. **分叉此儲存庫**：點擊 [這裡分叉](https://github.com/microsoft/MCP-Server-and-PostgreSQL-Sample-Retail/fork)
...
此範例展示如何構建和部署一個完整的 **Model Context Protocol (MCP) 伺服器**，為 AI 助手提供透過 PostgreSQL 安全且智能的零售銷售數據存取。該專案展示了企業級功能，包括 **行級安全性 (RLS)**、**語義搜索功能**以及 **Azure AI 整合**，適用於真實世界的零售分析場景。

**主要應用場景：**
- **AI 驅動的銷售分析**：使 AI 助手能夠透過自然語言查詢和分析零售銷售數據
- **安全的多租戶存取**：展示行級安全性實作，讓不同的店長僅能存取自己店鋪的數據
...
一個 Model Context Protocol (MCP) 伺服器，為 Zava 零售 DIY 業務提供全面的客戶銷售數據存取。此伺服器使 AI 助手能夠透過安全且結構化的介面查詢和分析零售銷售數據。

## 📚 完整實作指南

欲了解此解決方案的構建方式及如何實作類似的 MCP 伺服器，請參閱我們的完整 **[範例指南](Sample_Walkthrough.md)**。此指南提供：

- **架構深入解析**：元件分析與設計模式
- **逐步構建**：從專案設置到部署
- **代碼解析**：詳細解釋 MCP 伺服器的實作
- **進階功能**：行級安全性、語義搜索與監控
- **最佳實踐**：安全性、性能與開發指南
- **故障排除**：常見問題與解決方案
...
**Model Context Protocol (MCP)** 是一個開放標準，允許 AI 助手以安全方式即時存取外部數據源和工具。它是一座橋樑，讓 AI 模型能夠連接數據庫、API、文件系統及其他資源，同時保持安全性與控制。

### 主要優勢：
- **即時數據存取**：AI 助手能夠查詢即時數據庫和 API
- **安全整合**：透過身份驗證和權限控制存取
...
該指南涵蓋從基本 MCP 概念到進階生產部署的所有內容，包括：

- **MCP 基礎**：了解 Model Context Protocol 及其實際應用
- **資料庫整合**：實作安全的 PostgreSQL 連接與行級安全性
- **AI 增強功能**：使用 Azure OpenAI 嵌入添加語義搜索功能
- **安全性實作**：企業級身份驗證、授權與數據隔離
- **工具開發**：構建複雜的 MCP 工具以進行數據分析與商業智能
- **測試與除錯**：全面的測試策略與除錯技術
- **VS Code 整合**：配置 AI Chat 以進行自然語言數據庫查詢
- **生產部署**：容器化、擴展與雲端部署策略
- **監控與可觀察性**：應用洞察、日誌記錄與性能監控

### 學習路徑概覽

該指南遵循漸進式學習結構，適合各種技能水平的開發者：
...
| **[04-資料庫](walkthrough/04-Database/README.md)** | 數據層 | PostgreSQL 結構、pgvector、RLS 策略、範例數據 | 60 分鐘 |
...
**有經驗的開發者**：
1. 查看 [主要指南概覽](walkthrough/README.md)，了解模組摘要
...
- 之前的 MCP 經驗（我們會從零開始教）
- Azure 雲端經驗（我們提供逐步指導）
- 高階 PostgreSQL 知識（我們會根據需要解釋概念）

### 💡 學習技巧

1. **動手實作**：每個模組都包含可執行和修改的程式碼範例
...
部署腳本會自動部署 `text-embedding-3-small` 模型。在部署過程中，您可以選擇是否包含 `gpt-4o-mini` 模型。請注意，`gpt-4o-mini` **不是本專案的必要條件**，僅供未來可能的增強使用。
...
2. 使用 `.vscode/mcp.json` 中的配置啟動一個或多個 MCP 伺服器。該文件包含四個不同的伺服器配置，每個配置代表不同的商店管理角色：
   ...
   - 這模擬了真實世界中商店管理者使用不同 Entra ID 帳戶登入的情境
...
執行支援行級安全性的 PostgreSQL 銷售數據庫查詢。

**參數：**

- `postgresql_query` (str)：格式正確的 PostgreSQL 查詢

**返回值：** 查詢結果格式化為字符串（為了可讀性限制為 20 行）
...
| 商店位置 | RLS 使用者 ID | 描述 |
|----------|---------------|------|
| **全域存取** | `00000000-0000-0000-0000-000000000000` | 預設回退 - 所有商店存取 |
