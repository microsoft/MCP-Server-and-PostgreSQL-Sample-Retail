...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T16:23:51+00:00",
...
- **安全的多租戶存取**：展示 Row Level Security 的實作，讓不同的店長只能存取自己店鋪的數據
- **語義產品搜索**：展示使用文本嵌入的 AI 增強型產品搜索
- **企業整合**：展示如何將 MCP 伺服器與 Azure 服務及 PostgreSQL 數據庫整合

**適合對象：**
- 想學習如何建立 MCP 伺服器與資料庫整合的開發者
- 實作安全多租戶分析解決方案的數據工程師
- 使用零售或電子商務數據進行 AI 應用開發的開發者
- 對結合 AI 助手與企業數據庫感興趣的任何人

## 加入 Azure AI Foundry Discord 社群
分享您對 MCP 的使用經驗，並與專家和產品團隊交流
...
**Model Context Protocol (MCP)** 是一個開放標準，讓 AI 助手能夠安全地即時存取外部數據源及工具。它是一座橋樑，讓 AI 模型能夠連接數據庫、API、檔案系統及其他資源，同時保持安全性與控制。

### 主要優勢：
- **即時數據存取**：AI 助手能夠查詢即時數據庫及 API
- **安全整合**：透過身份驗證及權限控制存取
- **工具擴展性**：為 AI 助手新增自訂功能
...
了解基礎後，回到此處探索進階的零售分析實作！

## 📚 全面學習指南：/walkthrough

此儲存庫包含完整的 **12 模組學習導覽**，將此 MCP 零售伺服器範例拆解為易於理解的逐步課程。此導覽將此工作範例轉化為全面的教育資源，非常適合希望了解如何構建具備資料庫整合的生產級 MCP 伺服器的開發者。

### 您將學到什麼

此導覽涵蓋從 MCP 基本概念到進階生產部署的所有內容，包括：
...
**有經驗的開發者**：
1. 查看 [主要導覽概覽](walkthrough/README.md)，了解完整模組摘要
2. 跳到您感興趣的特定模組（例如模組 07：AI 整合）
3. 將各模組作為您自己專案的參考資料

**生產實作**：
1. 重點關注模組 02（安全性）、10（部署）及 11（監控）
...
1. 顯示各商店的銷售情況
1. 上季度各類別的銷售情況如何？
1. 我們銷售哪些與「油漆容器」類似的產品？

## 功能

- **多表架構存取**：在單次請求中檢索多個資料表的架構
- **安全查詢執行**：執行支援行級安全性的 PostgreSQL 查詢
- **即時數據**：存取當前銷售、庫存和客戶數據
- **日期/時間工具**：獲取當前 UTC 時間戳以進行時間敏感分析
- **靈活部署**：支援 HTTP 伺服器模式
...
對銷售資料庫執行支援行級安全性的 PostgreSQL 查詢。
