...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T19:02:19+00:00",
...
Этот модуль подробно рассматривает проектирование базы данных PostgreSQL для системы Zava Retail. Вы узнаете, как реализовать комплексную схему для розничной торговли с возможностями векторного поиска, многопользовательским моделированием данных и управлением доступом на уровне строк (Row Level Security, RLS) для изоляции данных.

## Обзор

База данных является основой нашего MCP-сервера, хранящей данные розничной торговли для нескольких магазинов при строгой изоляции данных. Мы используем PostgreSQL с расширением pgvector, чтобы обеспечить возможности семантического поиска, позволяя клиентам находить товары с помощью запросов на естественном языке.

Наша схема следует современным многопользовательским паттернам, а управление доступом на уровне строк гарантирует, что пользователи могут получать доступ только к данным своих авторизованных магазинов. Такой подход обеспечивает безопасность корпоративного уровня при сохранении оптимальной производительности.

## Цели обучения

К концу этого модуля вы сможете:

- **Проектировать** масштабируемые многопользовательские схемы базы данных для розничной торговли
- **Реализовать** PostgreSQL с pgvector для векторного поиска
- **Настроить** управление доступом на уровне строк для изоляции данных
...
Продолжите с **[Модуля 05: Реализация MCP-сервера](../05-MCP-Server/README.md)**, чтобы:

- Создать сервер FastMCP, который подключается к этой базе данных
- Реализовать инструменты запросов базы данных для протокола MCP
...
- [Архитектура многопользовательских данных](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Паттерны архитектуры Azure
...
- [Алгоритм HNSW](https://arxiv.org/abs/1603.09320) - Графы с иерархической навигацией малого мира

---

**Предыдущий**: [Модуль 03: Настройка окружения](../03-Setup/README.md)  
**Следующий**: [Модуль 05: Реализация MCP-сервера](../05-MCP-Server/README.md)
