...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T10:00:23+00:00",
...
3. **הצטרפו ל-Azure AI Foundry Discord**: [פגשו מומחים ומפתחים נוספים](https://discord.com/invite/ByRwuEEgH4)  
...
הדוגמה הזו מדגימה כיצד לבנות ולפרוס **שרת Model Context Protocol (MCP)** שמספק לעוזרים מבוססי AI גישה חכמה ומאובטחת לנתוני מכירות קמעונאיות דרך PostgreSQL. הפרויקט מציג תכונות ברמה ארגונית כמו **אבטחת רמות שורה (RLS)**, **יכולות חיפוש סמנטי**, ו**שילוב עם Azure AI** עבור תרחישי ניתוח קמעונאות בעולם האמיתי.

**שימושים מרכזיים:**
- **ניתוח מכירות מבוסס AI**: מאפשר לעוזרים מבוססי AI לשאול ולנתח נתוני מכירות קמעונאיות בשפה טבעית  
- **גישה מאובטחת רב-דיירית**: מדגים יישום אבטחת רמות שורה שבו מנהלי חנויות שונים יכולים לגשת רק לנתוני החנות שלהם  
- **חיפוש מוצרים סמנטי**: מציג גילוי מוצרים משופר באמצעות הטמעות טקסט  
- **שילוב ארגוני**: מדגים כיצד לשלב שרתי MCP עם שירותי Azure ומסדי נתונים PostgreSQL  

**מתאים במיוחד ל:**
- מפתחים הלומדים לבנות שרתי MCP עם שילוב מסדי נתונים  
- מהנדסי נתונים המיישמים פתרונות ניתוח מאובטחים רב-דייריים  
...
שרת Model Context Protocol (MCP) המספק גישה מקיפה למסד נתוני מכירות לקוחות עבור עסק Zava Retail DIY. שרת זה מאפשר לעוזרים מבוססי AI לשאול ולנתח נתוני מכירות קמעונאיות דרך ממשק מאובטח ומודע לסכמות.

## 📚 מדריך יישום מלא

לניתוח מפורט של איך הפתרון הזה נבנה ואיך ליישם שרתי MCP דומים, עיינו במדריך המלא **[Sample Walkthrough](Sample_Walkthrough.md)**. מדריך זה מספק:
...
- **תכונות מתקדמות**: אבטחת רמות שורה, חיפוש סמנטי, וניטור  
- **שיטות עבודה מומלצות**: אבטחה, ביצועים, והנחיות פיתוח  
- **פתרון בעיות**: בעיות נפוצות ופתרונות  

מתאים במיוחד למפתחים שרוצים להבין את פרטי היישום ולבנות פתרונות דומים.
...
- דוגמאות שילוב עם פלטפורמות AI פופולריות  
- משאבי קהילה ותמיכה  
...
ריפוזיטורי זה כולל **מדריך למידה בן 12 מודולים** שמפרק את דוגמת שרת MCP הקמעונאי הזה לשיעורים מעשיים, שלב-אחר-שלב. המדריך הופך את הדוגמה הזו למשאב חינוכי מקיף שמתאים במיוחד למפתחים שרוצים להבין איך לבנות שרתי MCP מוכנים לייצור עם שילוב מסדי נתונים.
...
- **שילוב עם VS Code**: הגדרת AI Chat לשאילתות מסדי נתונים בשפה טבעית  
- **פריסה בייצור**: קונטיינריזציה, סקיילינג, ואסטרטגיות פריסה בענן  
- **ניטור ותצפית**: Application Insights, לוגים, ומעקב ביצועים  

### סקירת מסלול הלמידה

המדריך עוקב אחר מבנה למידה מתקדם שמיועד למפתחים בכל רמות המיומנות:
...
| **[02-Security](walkthrough/02-Security/README.md)** | אבטחה ארגונית | אימות Azure, אבטחת רמות שורה, בידוד רב-דיירי | 60 דקות |
...
| **[06-Tools](walkthrough/06-Tools/README.md)** | פיתוח כלים | יצירת כלים MCP, אימות שאילתות, תכונות מודיעין עסקי | 75 דקות |
| **[07-Semantic-Search](walkthrough/07-Semantic-Search/README.md)** | שילוב AI | הטמעות Azure OpenAI, חיפוש וקטורי, אסטרטגיות חיפוש היברידי | 60 דקות |
...
| **[09-VS-Code](walkthrough/09-VS-Code/README.md)** | חוויית פיתוח | תצורת VS Code, שילוב AI Chat, זרימות ניפוי שגיאות | 45 דקות |
...
2. קפצו למודולים ספציפיים שמעניינים אתכם (לדוגמה, מודול 07 לשילוב AI)  
3. השתמשו במודולים בודדים כחומר עזר לפרויקטים שלכם  

**ליישום בייצור**:  
1. התמקדו במודולים 02 (אבטחה), 10 (פריסה), ו-11 (ניטור)  
...
**אפשרות 2: יישום מעשי** (קפצו ישר לבנייה)  
```bash
...
- הבנה כללית של מושגים ב-AI/ML
- ידע בסיסי בשורת הפקודה וב-Docker

**לא חובה (אבל מועיל)**:
- ניסיון קודם עם MCP (נלמד זאת מהבסיס)
- ניסיון בענן Azure (ניתן הדרכה שלב אחר שלב)
- ידע מתקדם ב-PostgreSQL (נסביר את המושגים לפי הצורך)

### 💡 טיפים ללמידה

1. **גישה מעשית**: כל מודול כולל דוגמאות קוד שניתן להריץ ולשנות
2. **מורכבות מתקדמת**: המושגים נבנים בהדרגה מפשוט למתקדם
3. **הקשר לעולם האמיתי**: כל הדוגמאות מבוססות על תרחישים עסקיים מציאותיים בתחום הקמעונאות
...
**מוכנים להתחיל ללמוד?** התחילו עם **[מודול 00: מבוא](walkthrough/00-Introduction/README.md)** או חקרו את **[סקירת המדריך המלאה](walkthrough/README.md)**.

## דרישות מקדימות

1. התקנת Docker Desktop
2. התקנת Git
3. **Azure CLI**: התקינו ואמתו את Azure CLI
...
הריצו את הסקריפטים הבאים כדי לבצע אוטומציה לפריסת משאבי Azure הנדרשים לשרת MCP.
...
הדרך הקלה ביותר להפעיל את כל המערכת (PostgreSQL + שרת MCP) היא באמצעות Docker Compose:
...
ההנחיות הבאות מניחות שתשתמשו בתמיכה המובנית של VS Code לשרת MCP.
...
2. הפעילו שרת MCP אחד או יותר באמצעות התצורות ב-`.vscode/mcp.json`. הקובץ מכיל ארבע תצורות שרת שונות, שכל אחת מייצגת תפקיד של מנהל חנות:
...
1. אילו מוצרים אנו מוכרים שדומים ל"כלי קיבול לצבע"
...
- `retail.orders` - הזמנות לקוחות ועסקאות
...
שליפת סכמות מאגר נתונים עבור טבלאות מרובות בבקשה אחת.
...
**תוצאות:** תוצאות השאילתה בפורמט מחרוזת (מוגבל ל-20 שורות לנוחות קריאה)
...
לכל מיקום חנות של Zava Retail יש מזהה משתמש RLS ייחודי שקובע אילו נתונים המשתמש יכול לגשת אליהם:
...
- נתוני מלאי עבור החנות הספציפית
...
- **בטיחות סוגים**: הקשר חזק מסוג עם מחלקת `AppContext`

### הקשר בקשה

- **חילוץ כותרות**: ניתוח כותרות מאובטח לזיהוי משתמש
- **אינטגרציית RLS**: פתרון מזהה משתמש אוטומטי מתוך הקשר הבקשה
...
2. **בידוד נתוני חנות**: מזהה משתמש RLS של כל חנות מבטיח גישה רק לנתוני החנות הזו
...
- **ניהול הקשר**: טיפול בהקשר יישום ובקשה עם בטיחות סוגים
...
3. עדכנו README זה עם כל תכונה או שינוי חדש
