...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T18:31:29+00:00",
  "source_file": "walkthrough/04-Database/README.md",
  "language_code": "fa"
}
-->
# طراحی پایگاه داده و طرح‌واره

## 🎯 آنچه این ماژول پوشش می‌دهد

این ماژول به طراحی پایگاه داده PostgreSQL برای سیستم خرده‌فروشی Zava می‌پردازد. شما یاد خواهید گرفت که یک طرح‌واره جامع خرده‌فروشی با قابلیت جستجوی برداری، مدل‌سازی داده‌های چند مستاجر و امنیت سطح ردیف (RLS) برای جداسازی داده‌ها پیاده‌سازی کنید.
...
طرح‌واره ما از الگوهای مدرن چند مستاجر پیروی می‌کند و امنیت سطح ردیف تضمین می‌کند که کاربران فقط می‌توانند به داده‌های فروشگاه‌های مجاز خود دسترسی داشته باشند. این رویکرد امنیت در سطح سازمانی را فراهم می‌کند و در عین حال عملکرد بهینه را حفظ می‌کند.

## اهداف یادگیری

تا پایان این ماژول، شما قادر خواهید بود:

- **طراحی** طرح‌واره‌های پایگاه داده خرده‌فروشی چند مستاجر مقیاس‌پذیر  
...
پایگاه داده از مدل چند مستاجر **پایگاه داده مشترک، طرح‌واره مشترک** با امنیت سطح ردیف استفاده می‌کند:
...
✅ **طراحی پایگاه داده چند مستاجر**: امنیت سطح ردیف را برای جداسازی امن داده‌ها پیاده‌سازی کرده باشید  
✅ **قابلیت‌های جستجوی برداری**: pgvector را برای جستجوی معنایی محصولات پیکربندی کرده باشید  
✅ **طرح‌واره جامع**: یک طرح‌واره پایگاه داده خرده‌فروشی آماده تولید ایجاد کرده باشید  
✅ **تولید داده‌های نمونه**: داده‌های آزمایشی واقعی برای توسعه و آزمایش ساخته باشید  
✅ **بهینه‌سازی عملکرد**: شاخص‌ها و بهینه‌سازی پرسش‌ها را پیکربندی کرده باشید  
✅ **پشتیبان‌گیری و بازیابی**: استراتژی‌های حفاظت از داده‌ها را ایجاد کرده باشید  

## 🚀 مرحله بعدی

ادامه دهید با **[ماژول ۰۵: پیاده‌سازی سرور MCP](../05-MCP-Server/README.md)** برای:

- ساخت سرور FastMCP که به این پایگاه داده متصل می‌شود  
- پیاده‌سازی ابزارهای پرسش پایگاه داده برای پروتکل MCP  
...
- [معماری داده چند مستاجر](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - الگوهای معماری Azure  
...
**قبلی**: [ماژول ۰۳: تنظیم محیط](../03-Setup/README.md)  
**بعدی**: [ماژول ۰۵: پیاده‌سازی سرور MCP](../05-MCP-Server/README.md)  
