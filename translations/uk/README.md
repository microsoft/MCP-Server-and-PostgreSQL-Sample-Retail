...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T10:27:48+00:00",
...
[![Приєднатися до Discord Azure AI Foundry](https://dcbadge.limes.pink/api/server/ByRwuEEgH4)](https://discord.com/invite/ByRwuEEgH4)
...
3. **Приєднання до Discord Azure AI Foundry**: [Зустрічайте експертів і розробників](https://discord.com/invite/ByRwuEEgH4)
...
- **Вирішення проблем**: Поширені проблеми та їх рішення
...
- **Безпечна інтеграція**: Контрольований доступ із автентифікацією та дозволами  
- **Розширюваність інструментів**: Додавання спеціальних функцій для AI-асистентів
- **Стандартизований протокол**: Працює на різних платформах і інструментах AI

### Новачок у MCP?

Якщо ви новачок у Model Context Protocol, рекомендуємо почати з комплексних ресурсів для початківців від Microsoft:
...
Цей репозиторій включає повний **12-модульний навчальний посібник**, який розбиває цей приклад сервера MCP для роздрібної торгівлі на зрозумілі покрокові уроки. Посібник перетворює цей робочий приклад на комплексний освітній ресурс, ідеальний для розробників, які хочуть зрозуміти, як створювати готові до виробництва сервери MCP з інтеграцією баз даних.
...
- **Інтеграція баз даних**: Реалізація безпечного підключення PostgreSQL із безпекою на рівні рядків
- **AI-покращені функції**: Додавання семантичного пошуку за допомогою векторів Azure OpenAI
- **Реалізація безпеки**: Автентифікація корпоративного рівня, авторизація та ізоляція даних
- **Розробка інструментів**: Створення складних MCP-інструментів для аналізу даних і бізнес-аналітики
- **Тестування та налагодження**: Комплексні стратегії тестування та методи налагодження
...
| **[02-Безпека](walkthrough/02-Security/README.md)** | Корпоративна безпека | Автентифікація Azure, безпека на рівні рядків, ізоляція для багатьох орендарів | 60 хвилин |
...
- Досвід роботи з хмарою Azure (ми надаємо покрокові інструкції)
...
2. **Поступова складність**: Концепції розвиваються поступово — від простих до складних
3. **Контекст реального світу**: Усі приклади базуються на реалістичних сценаріях роздрібного бізнесу
4. **Готовність до продакшну**: Приклади коду розроблені для використання в реальних умовах
...
- **[Azure AI Foundry](https://azure.microsoft.com/en-us/products/ai-foundry)**: Хмарна платформа, що використовується в прикладах
...
Найпростіший спосіб запустити повний стек (PostgreSQL + MCP Server) — використання Docker Compose:
...
Припускається, що ви будете використовувати вбудовану підтримку сервера MCP у VS Code.
...
2. Запустіть один або кілька серверів MCP, використовуючи конфігурації у файлі `.vscode/mcp.json`. Файл містить чотири різні конфігурації серверів, кожна з яких представляє роль менеджера магазину:
   ...
   - Кожна конфігурація використовує унікальний RLS (Row Level Security) ідентифікатор користувача
   - Ці ідентифікатори користувачів імітують різних менеджерів магазинів, які отримують доступ до бази даних
   - Система RLS обмежує доступ до даних на основі призначеного магазину менеджера
   - Це моделює реальні сценарії, коли менеджери магазинів входять у систему через різні облікові записи Entra ID
...
- **Доступ до багатотабличної схеми**: Отримання схем для кількох таблиць бази даних за один запит
...
Сервер забезпечує доступ до наступних таблиць бази даних роздрібної торгівлі:
...
- `retail.order_items` - Окремі позиції в замовленнях
...
Отримання схем бази даних для кількох таблиць за один запит.
...
**Результат:** Результати запиту у форматі рядка (обмежено 20 рядками для зручності читання)
...
Сервер реалізує Row Level Security, щоб гарантувати, що користувачі отримують доступ лише до даних, які вони мають право переглядати:
...
#### Унікальні RLS-ідентифікатори користувачів для магазинів

Кожен магазин Zava Retail має унікальний RLS-ідентифікатор користувача, який визначає, до яких даних користувач може отримати доступ:

| Локація магазину | RLS User ID | Опис |
|------------------|-------------|------|
...
Коли користувач підключається з конкретним RLS-ідентифікатором магазину, він бачить лише:

- Клієнтів, пов'язаних із цим магазином
- Замовлення, зроблені в цьому магазині
- Дані про запаси для цього конкретного магазину
- Метрики продажів і продуктивності магазину
...
Сервер використовує керований контекст додатка з:

- **Пулом підключень до бази даних**: Ефективне управління підключеннями для режиму HTTP
- **Управління життєвим циклом**: Правильне очищення ресурсів при завершенні роботи
- **Типова безпека**: Сильно типізований контекст із датакласом `AppContext`
...
- **Пул підключень**: Використовує асинхронні пули підключень для масштабованості
...
- **Перевірка таблиць**: Гарантує доступ лише до дійсних назв таблиць
...
2. **Ізоляція даних магазину**: RLS-ідентифікатор користувача кожного магазину забезпечує доступ лише до даних цього магазину
...
5. **Безпека підключення**: Використовуються безпечні практики підключення до бази даних
6. **Перевірка ідентичності користувача**: Завжди переконуйтеся, що використовується правильний RLS-ідентифікатор для потрібного магазину

### Важливі примітки щодо безпеки

- **Ніколи не використовуйте RLS-ідентифікатори користувачів продакшну в середовищах розробки**
- **Завжди перевіряйте, чи RLS-ідентифікатор користувача відповідає потрібному магазину перед виконанням запитів**
- **UUID за замовчуванням (`00000000-0000-0000-0000-000000000000`) забезпечує обмежений доступ**
- **Кожен менеджер магазину повинен мати доступ лише до RLS-ідентифікатора свого магазину**
...
- **FastMCP Server**: Сучасна реалізація сервера MCP із підтримкою асинхронності
- **PostgreSQL Provider**: Абстрактний рівень бази даних із підтримкою RLS
- **Управління контекстом**: Типізоване управління контекстом додатка та запиту
- **Реєстрація інструментів**: Декларативна реєстрація інструментів із валідацією Pydantic
