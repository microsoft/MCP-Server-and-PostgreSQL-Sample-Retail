...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T16:44:54+00:00",
...
本模块深入探讨 Zava 零售系统的 PostgreSQL 数据库设计。您将学习如何实现一个全面的零售架构，包括向量搜索功能、多租户数据建模以及用于数据隔离的行级安全（RLS）。

## 概述

数据库是我们 MCP 服务器的基础，负责存储多个商店的零售数据，同时保持严格的数据隔离。我们使用 PostgreSQL 和 pgvector 扩展来实现语义搜索功能，使客户能够通过自然语言查询找到产品。

我们的架构遵循现代多租户模式，并通过行级安全确保用户只能访问其授权商店的数据。这种方法提供了企业级的安全性，同时保持了最佳性能。
...
- **设计** 可扩展的多租户零售数据库架构
- **实现** PostgreSQL 和 pgvector 的向量搜索功能
- **配置** 行级安全以实现数据隔离
- **生成** 真实的测试样本数据
- **优化** 零售工作负载的数据库性能
- **实施** 数据库备份和恢复策略
...
我们的数据库结合了 PostgreSQL 的企业功能和 pgvector 扩展，以支持 AI 驱动的搜索：
...
数据库采用 **共享数据库，共享架构** 的多租户模型，并启用了行级安全：
...
✅ **多租户数据库设计**：实现行级安全以确保数据隔离  
✅ **向量搜索功能**：配置 pgvector 以支持语义产品搜索  
✅ **全面架构设计**：创建生产级零售数据库架构  
✅ **样本数据生成**：构建真实的测试数据以支持开发和测试  
✅ **性能优化**：配置索引并优化查询性能  
✅ **备份与恢复**：建立可靠的数据保护策略  

## 🚀 下一步

继续学习 **[模块 05：MCP 服务器实现](../05-MCP-Server/README.md)**，以：

- 构建连接到此数据库的 FastMCP 服务器
- 实现 MCP 协议的数据库查询工具
- 添加基于嵌入的语义搜索功能
- 配置连接池和错误处理
...
### 向量数据库
- [向量搜索基础](https://www.pinecone.io/learn/vector-database/) - 了解向量数据库
- [嵌入模型](https://platform.openai.com/docs/guides/embeddings) - OpenAI 嵌入文档
- [HNSW 算法](https://arxiv.org/abs/1603.09320) - 分层可导航小世界图

---

**上一模块**: [模块 03：环境设置](../03-Setup/README.md)  
**下一模块**: [模块 05：MCP 服务器实现](../05-MCP-Server/README.md)
