...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T16:44:08+00:00",
...
本模块提供了关于如何使用现代容器化和云原生方法将 MCP 零售服务器部署到生产环境的全面指导。您将学习如何部署可扩展、安全且可监控的 MCP 服务器，以处理企业级工作负载。

## 概述

MCP 服务器的生产部署需要仔细考虑容器化、编排、安全性、可扩展性和监控。本模块涵盖了使用 Azure Container Apps 和 PostgreSQL Flexible Server 进行部署、实施 CI/CD 流水线，以及为可变工作负载配置自动扩展。

部署策略从用于开发的简单单容器部署到复杂的多区域、自动扩展的生产环境，配备全面的监控和安全功能。

## 学习目标

完成本模块后，您将能够：

- 使用 Docker 和多阶段构建对 MCP 服务器进行 **容器化**
...
继续学习 **[模块 11：监控和可观察性](../11-Monitoring/README.md)**，以：

- 设置全面的监控功能，使用 Application Insights
- 配置结构化日志记录和分布式追踪
- 实现告警和自动响应系统
- 监控业务指标和性能 KPI
...
**上一模块**: [模块 09：VS Code 集成](../09-VS-Code/README.md)  
**下一模块**: [模块 11：监控和可观察性](../11-Monitoring/README.md)
