...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T16:22:43+00:00",
...
一个模型上下文协议（MCP）服务器，为Zava零售DIY业务提供全面的客户销售数据库访问。此服务器通过安全、模式感知的接口使AI助手能够查询和分析零售销售数据。
...
如果您是模型上下文协议的新手，我们建议从微软的全面入门资源开始：
...
了解基础知识后，返回此处探索高级零售分析实现！

## 📚 综合学习指南：/walkthrough

此仓库包含完整的**12模块学习演练**，将此MCP零售服务器示例分解为易于理解的分步课程。演练将此工作示例转化为全面的教育资源，非常适合希望了解如何构建带数据库集成的生产级MCP服务器的开发者。
...
**对于有经验的开发者**：
1. 查看[主演练概览](walkthrough/README.md)，了解完整模块摘要
2. 跳到您感兴趣的特定模块（例如模块07：AI集成）
3. 将单个模块作为您自己项目的参考材料

**对于生产实施**：
1. 重点关注模块02（安全）、10（部署）和11（监控）
...
- **[示例演练](Sample_Walkthrough.md)**：高层次技术概述
...
**准备好开始学习了吗？** 从 **[模块 00：简介](walkthrough/00-Introduction/README.md)** 开始，或者浏览 **[完整演练概述](walkthrough/README.md)**。
...
- **多表模式访问**：在单次请求中检索多个数据库表的模式
- **安全查询执行**：支持行级安全（RLS）的 PostgreSQL 查询执行
- **实时数据**：访问当前的销售、库存和客户数据
- **日期/时间工具**：获取当前 UTC 时间戳以进行时间敏感分析
- **灵活部署**：支持 HTTP 服务器模式
...
- `retail.stores` - 门店位置和详细信息
- `retail.categories` - 产品类别和层次结构
...
- `table_names` (list[str])：支持表中有效表名的列表

**返回值：** 请求表的拼接模式字符串

### `execute_sales_query`

使用行级安全（RLS）对销售数据库执行 PostgreSQL 查询。
...
服务器实现了行级安全，以确保用户只能访问他们被授权查看的数据：
...
当用户使用特定门店的 RLS 用户 ID 连接时，他们将仅看到：
...
这确保了不同门店位置之间的数据隔离，同时保持统一的数据库模式。
...
*此 MCP 服务器为 AI 驱动的分析和洞察提供了对 Zava 零售销售数据的安全高效访问。*
